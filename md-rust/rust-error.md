...
=================================
# 5.unresolved import `tokio`

`unresolved import 'tokio'` 错误通常表示 Rust 项目中未正确引入 **Tokio 依赖**。以下是解决步骤和示例：


### 一、**添加 Tokio 依赖到 `Cargo.toml`**
1. **基础配置（默认功能）**：
   ```toml
   [dependencies]
   ...
   - **按需选择功能**（减少编译时间）：
     ```toml
     tokio = { version = "1", features = ["macros", "rt-multi-thread"] }  # 常用功能
     ```
...
确保正确导入 Tokio 模块：
```rust
use tokio::task;  // 导入特定模块
use tokio::time::{sleep, Duration};  // 导入子模块
...
### 三、**常见错误原因**
1. **依赖未更新**：  
   修改 `Cargo.toml` 后未执行 `cargo build` 或 `cargo update`。

2. **版本冲突**：  
   检查是否存在多个版本的 Tokio 依赖（通过 `cargo tree | grep tokio` 查看）。

3. **IDE 缓存问题**：  
   - 重启 VS Code/Rust Analyzer。  
   - 清除 Cargo 缓存：`cargo clean` 后重新构建。

4. **Cargo 配置错误**：  
   确保 `Cargo.toml` 中没有拼写错误，且依赖位于正确的 `[dependencies]` 部分。
...
2. **异步 I/O 优化**：
   ```toml
   tokio = { version = "1", features = ["fs", "net", "macros"] }  # 按需选择
   ```


### 六、**相关工具链检查**
1. **Rust 版本**：  
   确保使用稳定版 Rust（Tokio 1.x 支持 Rust 1.65+）：
   ```bash
   rustup update stable
   ```

2. **Cargo 配置**：  
   检查 `~/.cargo/config.toml` 是否有自定义源或代理配置。


若问题依旧存在，请提供完整的 `Cargo.toml` 和错误日志，以便进一步诊断！