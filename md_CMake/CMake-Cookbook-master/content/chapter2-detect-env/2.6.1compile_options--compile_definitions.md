# 1. target_compile_options  target_compile_definitions
...
## 1.2.target_compile_definitions
### 1.2.1功能：
...
## 1.4.使用场景
### 1.4.1target_compile_options 使用场景：
...
### 1.4.2target_compile_definitions 使用场景：
...
=============================================================================================
# 2.target_compile_definitions(testEXE PRIVATE -DUSE_SSE2 -DNO_WARN_USE_SSE2)和target_compile_definitions(testEXE PRIVATE  USE_SSE2  NO_WARN_USE_SSE2)作用一样吗

在 CMake 中，`target_compile_definitions()` 的两种写法 **作用不完全相同**，具体区别如下：
...
#### **(1) 带 `-D` 前缀（推荐）**
```cmake
target_compile_definitions(testEXE PRIVATE -DUSE_SSE2 -DNO_WARN_USE_SSE2)
```
- **作用**：  
  明确通过 `-D` 定义宏，等价于 GCC/Clang 的 `-DUSE_SSE2 -DNO_WARN_USE_SSE2`。  
- **生成的编译命令**：  
  ```bash
  g++ ... -DUSE_SSE2 -DNO_WARN_USE_SSE2 ...
  ```

#### **(2) 不带 `-D` 前缀**
```cmake
target_compile_definitions(testEXE PRIVATE USE_SSE2 NO_WARN_USE_SSE2)
```
- **作用**：  
  CMake 会自动为每个标识符添加 `-D` 前缀，最终效果与第一种写法相同。  
- **生成的编译命令**：  
  ```bash
  g++ ... -DUSE_SSE2 -DNO_WARN_USE_SSE2 ...
  ```
...
| **行为**                | **带 `-D` 的写法**               | **不带 `-D` 的写法**             |
|-------------------------|----------------------------------|----------------------------------|
| **可读性**              | 更接近原生编译器选项，直观        | 更简洁，但可能混淆宏与普通变量    |
| **兼容性**              | 明确指定，避免歧义                | 依赖 CMake 自动处理              |
| **特殊字符处理**        | 需手动转义（如 `-D"MY_DEF=value"`）| CMake 自动处理简单情况           |
| **跨平台一致性**        | 更可靠（如 MSVC 需 `/D` 替代 `-D`）| CMake 会适配平台，但可能需额外配置 |

---

### **3. 实际建议**
- **推荐使用带 `-D` 的写法**：  
  显式声明更清晰，尤其在定义带值的宏时（如 `-DUSE_SSE2=1`）或需要转义时。
  ```cmake
  target_compile_definitions(testEXE PRIVATE -DUSE_SSE2=1 -DNO_WARN_USE_SSE2)
  ```

- **不带 `-D` 的场景**：  
  仅适用于简单的布尔宏（无值、无特殊字符），依赖 CMake 的自动转换。

---

### **4. 验证方式**
通过以下命令查看生成的编译指令，确认两者是否一致：
```bash
cmake --build . --verbose
```
或直接检查生成的 `build.ninja`/`Makefile` 文件。

---

**结论**：两种写法在简单情况下效果相同，但显式使用 `-D` 更可靠且可读性更强，尤其在复杂项目中。