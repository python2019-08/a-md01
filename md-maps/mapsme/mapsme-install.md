...
- **Abner Remark**: git clone --depth=1 --single-branch --recurse-submodules -b release/24.11.12  https://github.com/organicmaps/organicmaps.git mapsme-organicmaps2024-1112
...
## Abner :在windows上，以系统管理员身份打开Git bash ，然后在Git bash 运行下面的git命令。
## 该Git已经在安装时Enable [symlinks] support. 
git clone --depth=1 --single-branch --recurse-submodules -b release/24.11.12  https://github.com/organicmaps/organicmaps.git mapsme-organicmaps2024-1112
```
...
```bash
"C:\Program Files\Git\bin\bash.exe" configure.sh # execute the script by using VS Developer Command Prompt
```

### 2.1 Special cases options
...
##### 3.2.1.1 Fully supported versions
...
##### 3.2.1.2 Workarounds for older Ubuntu versions
...
#### 4.6.1 Building from the command line
...
#### 4.6.3 Enable Vulkan Validation
